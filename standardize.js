...
async function createVideoData(data) {
    const videoRecord = {
        "videoid": data[0].video.videoid,
        "title": data[0].video.title,
        "viewcount": data[0].video.viewcount,
        "duration": data[0].video.duration,
        "thumbnailurl": "",
        "category": data[0].video.category,
        "uploaddate": data[0].video.uploaddate,
        "channelid": data[0].video.channelid,
        "channelname": data[0].video.channelname
    }
...
async function createSubtitleData([videoData, originalData]) {
    ...
    const englishCaption = originalData[0].captions.find((caption) => (caption.lang === "en")).subtitles.transcript.text;
